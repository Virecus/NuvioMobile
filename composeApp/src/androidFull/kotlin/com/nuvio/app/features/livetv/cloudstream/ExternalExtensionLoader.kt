 ...
 * Ported from NuvioTV (Hilt removed; CloudStream runtime init delegated to [CloudstreamRuntime]).
 */
class ExternalExtensionLoader(
    ...
    suspend fun downloadExtension(scraperId: String, downloadUrl: String): File? = withContext(Dispatchers.IO) {
        CloudstreamRuntime.ensureInitialized()
        try {
                    ...
                    for (className in candidates) {
                        try {
                            ...
                            if (apis.isEmpty()
                                ...
                                && !clazz.isInterface) {
                                val instance = tryInstantiateMainApi(clazz) ?: continue
     ...
     * Unsafe.allocateInstance when the constructor fails.
     */
    private fun tryInstantiateMainApi(clazz: Class<*>): MainAPI? {
        try {
